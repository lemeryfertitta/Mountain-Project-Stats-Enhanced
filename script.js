(function () {
    ...
    rows.sort((a, b) => {
      if (sortBy === "Date") {
        const dateA = new Date(
          a.querySelector("td:last-child div").textContent
        );
  ...
  document.addEventListener("DOMContentLoaded", () => {
      ...
      for (const mutation of mutationsList) {
        for (const node of mutation.addedNodes) {
          ...
          if (
            ...
            // Add date column to stars, ratings, and todos tables
            const td = document.createElement("td");
            ...
            node.children[STARS_INDEX].classList.replace(
              "col-lg-2",
              "col-lg-3"
            );
            ...
            node.children[TICKS_INDEX].classList.replace(
              "col-lg-6",
              "col-lg-4"
            );
          }
    ...
    if (statsTable) {
      ...
      console.error("MP Stats Extended: stats table not found on route page.");
    }
  });
})();
